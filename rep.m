...
%             heatMapBonitoDilatado = imdilate(hmfb,SE); %dilata a figura.
    ...
    if e(1).groupInfo
        for i=1:length(t)
            ...
            %centroid
            meanx(i) = mean(posicao{:}.x(i));
            meany(i) = mean(posicao{:}.y(i));
    ...
    if e(1).groupInfoVsanimalOne
        for i =1:length(t)
            %variances
            varx(i) = var(posicao{2:end}.x(i));
            vary(i) = var(posicao{2:end}.y(i));
            %centroid
            meanx(i) = mean(posicao{2:end}.x(i));
            meany(i) = mean(posicao{2:end}.y(i));
